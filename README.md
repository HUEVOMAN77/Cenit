...
[![Última versión](https://img.shields.io/badge/versión%20actual-0.6.20-informational)](https://github.com/HUEVOMAN77/Cenit/releases/tag/base-0.6.20)
[![Driver](https://img.shields.io/badge/driver%20incluido-Turnip%2025.3.6-yellow)](https://github.com/HUEVOMAN77/Cenit/releases)

**Cenit** es un emulador de PlayStation 2 para celulares Android, **creado y mantenido por una sola persona: [HUEVOMAN77](https://github.com/HUEVOMAN77).** Nace como proyecto independiente sobre la base de [PSX2](https://github.com/izzy2lost/PSX2) (el adaptador Android de PCSX2 2.7), pero todo lo que define a Cenit —el motor de rendimiento adaptativo, los perfiles de hardware, los ajustes por juego, el driver Vulkan propio, la interfaz, la instrumentación del recompilador VU— es trabajo original de su autor, pieza por pieza y en público.
...
No es una copia con otro logo. Cada versión suma mejoras de rendimiento y de uso que no existen ni en PSX2 ni en PCSX2, y —salvo que se diga lo contrario— medidas sobre hardware real antes de publicarse.

> **Versión actual: 0.6.20.** El proyecto mantiene una línea pública y continua (0.6.x), con su release, su verificación en teléfono real y su registro de qué cambió y qué no. En [Historial versión por versión](#historial-versión-por-versión) está todo el camino recorrido, sin adornos.
...
| | |
|---|---|
| **Versión actual** | 0.6.20 |
| **Descarga** | [Lanzamientos](https://github.com/HUEVOMAN77/Cenit/releases) → `Cenit-0.6.20.apk` |
| **Driver incluido** | `Cenit-Turnip-25.3.6` (compilado por el CI de este repo) |
...
Regla permanente del proyecto: **cada release muestra siempre los dos archivos** —el APK del emulador y el driver más reciente—, sin importar cuál de los dos se haya publicado en ese momento. Así, desde la página que abras puedes instalar el emulador e importar el driver sin buscar más.
...
| Marca | Estado |
|---|---|
| **Qualcomm Snapdragon (Adreno)** | Soporte completo: perfil de rendimiento dedicado, detección del modelo exacto (incluso con los apodos internos que muchas marcas usan), controladores Turnip importables y **ajuste fino propio del driver** desde 0.6.20. |
| **MediaTek Helio / Dimensity (Mali)** | Detectado y clasificado por gama —del Helio G/P económico al Dimensity tope de línea— con su perfil de rendimiento. El ajuste fino específico de Mali está en el camino. |
| **Samsung Exynos** | Detectado y clasificado por gama, del Exynos 7870 al 2400, con perfil según su potencia real. |
| **Huawei Kirin** | Detectado y clasificado; al ser un ecosistema con menos presencia global, su ajuste fino llega más adelante. |
| **Otros (Google Tensor, etc.)** | Si el teléfono corre Android de 64 bits, Cenit funciona con el perfil conservador de gama baja hasta tener clasificación propia. |

La regla: si tu procesador no está catalogado, se asume el perfil más seguro para no prometer rendimiento que no puede dar, y se agrega en cuanto hay mediciones reales.
...
- **Perfil automático por hardware.** Al instalar, reconoce el procesador —incluidos los apodos internos de Qualcomm que muchas marcas reportan mal— y deja lista la línea base: aceleradores de CPU seguros, recompilación completa, sin esperas inútiles a la gráfica. Arranca ya configurado para tu gama.
...
- **Arreglos automáticos de compatibilidad.** El motor aplica las correcciones conocidas de cada juego, y ninguna función de Cenit las rompe al tocar ajustes manuales.
...
Éste es el bloque más reciente del proyecto y el que más cambió la forma en que Cenit habla con la GPU.
...
- **Correcciones finas horneadas (driconf).** En Android el driconf **no se lee de un archivo en tiempo de ejecución**: se compila dentro del driver. Cenit ya exporta el archivo de reglas y el CI lo hornea en la siguiente compilación. El diálogo te dice con claridad qué parte de una regla está viva ahora y qué parte necesita recompilar — no se te vende como activo lo que no lo es.
...
- **Guardarraya más justo.** Si un juego se cierra con el ajuste fino de Cenit activado, lo primero que se retira es **el ajuste fino de ese juego**, no el driver. El driver solo se condena si vuelve a fallar ya limpio. Así no se le echa la culpa a un controlador por un error nuestro.

> **Sobre promesas de rendimiento.** Ni este driver ni ningún perfil se anuncian como «más rápido» sin datos. La ganancia esperable y verificable hoy es la de la caché de shaders (menos compilación repetida, arranque más suave). Lo demás requiere tu medición en tu teléfono, y para eso está el medidor.

### 4. Instrumentación: medir antes de tocar

Cenit no cambia el motor "a ver si suena". Desde 0.6.13 el proyecto viene construyendo la evidencia primero:
...
- **Diagnóstico de afinidad (0.6.13).** El log de arranque imprime, para EE/VU1/GS, no solo el índice del núcleo sino su **clúster y frecuencia máxima**, para responder con datos del teléfono concreto si el hilo del juego cayó de verdad en el núcleo rápido.
- **Auditoría de compilación en el CI.** El job genera `compile_commands.json` y vuelca en el log los flags reales de los archivos calientes (microVU, iR5900) más el tamaño de los `.so`. Release/LTO se auditan con evidencia, no con suposiciones. Se confirmó así que el core ya compila a −O3: ahí no había margen que buscar.
...
1. Descarga `Cenit-0.6.20.apk` desde [Lanzamientos](https://github.com/HUEVOMAN77/Cenit/releases) e instálala (Android pedirá permitir apps de esta fuente la primera vez).
...
Para contar un problema o pedir una mejora, usa la pestaña **[Issues](https://github.com/HUEVOMAN77/Cenit/issues)**. No necesitas saber programar ni escribir en inglés; alcanza con contarlo en español. Para que el reporte sirva de verdad, trata de incluir:
...
Los reportes de juegos específicos son la materia prima con la que se construye la tabla de rendimiento y el ajuste fino por marca de procesador. Si tu gama baja corre algo que antes era injugable, también vale la pena contarlo.

---

## Historial versión por versión

Todo el camino, incluido lo que estuvo mal y se corrigió. Cada fila tiene su release con sus archivos.
...
| **driver 2026-09-24** | Nace **Cenit Turnip**: primer driver compilado por nuestro CI (Mesa 25.2.8) en formato adrenotools con `vulkan.cenit.so` propio y símbolos aparte para diagnosticar. |
...
| **0.6.20** | **Ajuste fino del driver (tres niveles).** Driver sobre **Mesa 25.3.6 + ThinLTO**; perfiles automáticos por gama; **caché de shaders en disco** activada para drivers importados; **reglas por juego** (banderas activas ya, correcciones driconf horneadas en la siguiente compilación); **medidor de FPS por configuración realmente aplicada**; y guardarraya que retira el ajuste fino antes de condenar un driver. |

---

## Hoja de ruta

Lo que sigue es el plan honesto: parte es ingeniería difícil, parte es directamente ambiciosa, y **nada de esto se anuncia como «ya funciona»**.

### Plan inmediato

1. **Cosechar la Fase 1 de la sonda VU.** El ranking de fusión ya se calcula; lo que falta son los reportes de teléfonos reales para saber qué aristas valen la pena. Con eso se decide la Fase 2 — **fusión de superbloques VU**, el trabajo más prometedor contra el cuello de CPU— que entrará con su bandera **apagada por defecto** y medirse-antes-de-encenderla, como todo lo demás del proyecto. Sin datos tuyos, la Fase 2 no se toca.
2. **Reglas por juego que se vuelvan configuración de fábrica.** Cada regla que demuestre ganar en el medidor se hornea en el siguiente Cenit Turnip, para que el beneficio llegue a todos sin que nadie edite nada. El canal ya está construido en 0.6.20.
...
4. **Ajuste fino por marca de procesador:** perfiles específicos para MediaTek/Mali y Samsung/Exynos, con la misma lógica de medición que ya usa Snapdragon.
...
La idea central de las próximas versiones grandes: **que el juego renderice por debajo de la resolución nativa de PS2 —incluso 0.5×, la más baja— y que Cenit reconstruya esa imagen en tiempo real para mostrarla nítida a 720p u 1080p en la pantalla del celular.**
...
**Estabilidad continua:** cada versión pasa por compilación y verificación automatizadas antes de publicarse, y el driver se compila desde la fuente oficial en el mismo CI, con sus símbolos guardados para poder leer cualquier cierre.

---

## Cómo trabaja este proyecto

Cuatro reglas que se pueden auditar en el historial de commits:
...
2. **Todo se puede apagar.** Cada mecanismo propio tiene su interruptor y su explicación en español, porque tu teléfono manda más que cualquier perfil general.
3. **Sin promesas sin datos.** Si algo no está medido en hardware real, se describe como esperado o como experimental, nunca como arreglo. También se dicen en público los bugs y las versiones que no funcionaron (0.6.8, 0.6.9 y el INI por-juego hasta 0.6.14 están ahí con nombre y apellido).
...
Todo lo demás —la identidad, la interfaz, el motor de rendimiento adaptativo, los ajustes por juego, los perfiles de hardware, el guardarraya, el ajuste fino del driver y la sonda y futuro motor de superbloques VU— es de Cenit y de su autor.
